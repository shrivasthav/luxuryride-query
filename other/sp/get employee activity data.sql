CREATE DEFINER=`root`@`%` PROCEDURE `getactivitydata`(in showroom_id_set int,
 in start_date varchar(25),in end_date varchar(25))
begin
...
count(distinct if(a.customer_status=3 and a.followup_status=6 and a.follow_date_time between start_date and end_date
 ,a.customer_id,NULL)) as booked,
 count(distinct if(a.customer_status=3 and a.followup_status=3 and a.follow_date_time between start_date and end_date
 ,a.customer_id,NULL)) as test_drive,
  ...
  between start_date and end_date,a.customer_id,NULL)) as closed,
count(distinct if(a.customer_status=7 and a.follow_date_time between start_date and end_date
...
count(distinct if(date(a.inactive_on) between start_date and end_date,a.customer_id,NULL)) as detailleads
from 
